...
push @tests, [ 'Opportunistic TLS but not secure', sub {
	my $s = Net::SMTP::Extended->new($host,
		Port	=> $port,
		Hello	=> 'localhost',
	...
	ok (ref $s eq 'Net::SMTP::Extended::TLS', "TLS");
	ok ($s->isa('Net::SMTP'), "Is a Net::SMTP");
	ok ($s->isa('IO::Socket::SSL'), "Is a IO::Socket::SSL");

	ok ($s->peerhost eq $host, "peerhost is $host");
	ok ($s->peerport eq $port, "peerport is $port");

	ok ($s->is_secure, "is secure!");


	ok ($s->can('banner'), "can banner");
	diag("banner: "  . $s->banner());
	ok ($s->can('domain'), "can domain");
	diag("domain: "  . $s->domain());
       #
